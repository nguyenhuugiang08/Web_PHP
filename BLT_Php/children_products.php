...
$sql = "select *from product where category_id = 5 limit 7";
$listProduct = executeResult($sql);

$sql = "select count(id) as countId from product where category_id = 5 ";
        ...
        <div class="col-md-3">
            ...
            <div class="d-flex justify-content-center align-items-center mb-5">
                ...
                <div class="display">
                    ...
                    <span id="min" class="mx-2">750,000đ</span>
                    <span>-</span>
                    <span id="max" class="ms-2">900,000đ</span>
                </div>
            </div>
        ...
        <div class="col-md-9">
            ...
            <nav id="pagination" aria-label="Page navigation example">
                <ul class="pagination">
                    ...
                    for ($i = 1; $i <= $pages; $i++) {
                        echo '
                            <li class="page-item"><a class="page-link me-1 page-link-'.$i.' '.($i ==1 ? "active-link" : "").' "
                             onclick = "currentPage('.$i.')">' . $i . '</a></li>
                        ';
                    }
                    ?>
                    <li class="page-item">
...
<script>
    ...
    $('#rangeMin').on('input', function(e) {
        ...
        $('#min').html((newValue * 1500 + 750000).toLocaleString('en-US') + "đ");
        $('#line').css({
    ...
    $('#rangeMax').on('input', function(e) {
        ...
        $('#max').html((newValue * 1500 + 750000).toLocaleString('en-US') + "đ");
        $('#line').css({
    ...
    function currentPage(currentPage) {
        ...
        let pageElement = document.querySelector('.page-link-'+currentPage)
        pageElement.classList.add('active-link')

         $('.page-link').filter(x => x !== currentPage).removeClass('active-link')
    }

    // post data to filter by values of input range
    let valueRangeMin = 750000
    let valueRangeMax = 900000

    $('#rangeMin').on('change input', (e) => {
         valueRangeMin = e.target.value * 1500 + 750000 
    });

    $('#rangeMax').on('chane input', (e) => {
         valueRangeMax = e.target.value * 1500 + 750000
    });
    ...
    function currentFilterPage(filterPage) {
        ...
        let pageElement = document.querySelector('.page-link-'+filterPage)
        pageElement.classList.add('active-link')

         $('.page-link').filter(x => x !== filterPage).removeClass('active-link')
    }
    // prev page
    function prevPage() {
       let prevPage = $('.active-link')[0].innerText - 1
       prevPage != 0 && currentPage(prevPage)
    }
    // next page
    function nextPage() {
       let nextPage = Number($('.active-link')[0].innerText) + 1
       nextPage <= $('.page-link').length - 2 && currentPage(nextPage)
    }
</script>
