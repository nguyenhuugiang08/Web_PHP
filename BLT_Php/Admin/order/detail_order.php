...
<div class="product-process">
    <div class="my-3 ms-4">THÔNG TIN KHÁCH HÀNG</div>
    <div class="row mb-2 ms-3">
        <div class="col-md-3">Họ tên:</div>
        <div class="col-md-3"><?= $result[0]['fullname'] ?></div>
    </div>

    <div class="row mb-2 ms-3">
        <div class="col-md-3">Ngày đặt hàng:</div>
        <div class="col-md-3"><?= $result[0]['order_date'] ?></div>
    </div>

    <div class="row mb-2 ms-3">
        <div class="col-md-3">Số điện thoại:</div>
        <div class="col-md-3"><?= $result[0]['phone_number'] ?></div>
    </div>
    <div class="row mb-2 ms-3">
        <div class="col-md-3">Địa chỉ:</div>
        <div class="col-md-3"><?= $result[0]['address'] ?></div>
    </div>

    <div class="row mb-4 ms-3">
        <div class="col-md-3">Ghi chú:</div>
        <div class="col-md-3"><?= $result[0]['note'] ?></div>
    </div>
    ...
    background-image: repeating-linear-gradient(45deg, rgb(111, 166, 214), rgb(111, 166, 214) 33px, transparent 0px, transparent 41px, rgb(241, 141, 155) 0px, rgb(241, 141, 155) 74px, transparent 0px, transparent 82px);"></div>
    <?php
    foreach ($result as $item) {
        $sql = "select *from order_details inner join orders INNER join product 
                            where orders.id = order_details.order_id and order_details.product_id = product.id and order_id = $id";
        $data = executeResult($sql);
        echo '
        <div class="row p-5" style = "background-color: #FFFCF5;">
        '
    ?>
        <?php
        foreach ($data as $item) {
            echo '
                <div class="col-md-12 d-flex justify-content-between align-items-center border-bottom border-1 py-3">
...
<div class="d-flex justify-content-end mt-5 mb-3">
    <a href="admin_orders.php">
        <button class="btn btn-primary ms-5">Trở Lại</button>
    </a>
</div>
</div>
